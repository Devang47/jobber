...
class FakeNotifier:
    ...
    async def send_job_with_proposal(self, job_card, proposal, job_id="", url="", chat_id=None):
        self.job_messages.append((chat_id, job_card, proposal, url))
        if job_id:
...
class JobBotTests(unittest.IsolatedAsyncioTestCase):
        ...
        async def fetch_reddit(seen_ids):
            ...
            return [
                PlatformJob(
                    ...
                    description="Need a React developer to build a dashboard.",
    ...
    async def test_start_and_stop_manage_schedules(self):
        await self.bot.handle_command("/start reddit", "42", 42)
        self.assertTrue(self.store.is_subscribed(42, "reddit"))
        self.assertIn((42, "reddit"), self.bot.scheduled_jobs)

        await self.bot.handle_command("/stop reddit", "42", 42)
